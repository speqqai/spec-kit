...
Add `--scope user` to register it for every project instead of just the current one. Then start a session, run `/mcp`, and authenticate the `speqq` server — your browser opens a Speqq login. Confirm with `claude mcp list`.
...
Add the Speqq server to the harness's MCP configuration file (project-level or global) with the URL above; use the harness's OAuth flow when it offers one, or the bearer-token form below when it does not. See that harness's MCP documentation for the exact schema.
